...
for participant in participants:
    ...
    for intensity in intensities:
        ...
        # def comy_by_row(row, com_local=com):
        #     return [com_local['CoM pos y'][row['start_frame'] : row['stop_frame']].values - com_local['CoM pos y'][row['start_frame']]]
        ...
        for tech in set(slint['subtech'].values):
            ...
            for i in range(len(slint['com_pos_z'][slint['subtech'] == tech])):
                # y = slint['com_pos_x'][slint['subtech'] == 'dp'].values[i][0]
            ...
            tech_dict[tech] = pd.Series(np.std(subtech_df, axis=1), name=tech)
...
com_df.to_csv(working_dir/'std_com_height.csv')
